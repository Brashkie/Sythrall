...
@router.post("/project")
async def analyze_project(req: AnalyzeProjectRequest) -> dict[str, Any]:
    """
    Igual que /code, pero para un proyecto entero de una sola vez.

    /code corre flake8+pylint como subprocess POR ARCHIVO — bien para un archivo
    suelto, pero con miles de archivos ese arranque de intérprete repetido domina
    el tiempo total (~0.9s/archivo medido, ~15min proyectados a 1000 archivos).
    Acá se corren UNA vez cada uno, pasándoles todos los paths juntos — flake8 y
    pylint ya soportan múltiples archivos en una sola invocación. Mismo benchmark:
    200 archivos pasó de ~180s a ~5.6s. Ver CHANGELOG.md para el detalle.

    Trade-off conocido: el "pylint_score" (X/10) que /code sí da por archivo acá
    queda en None — pylint solo calcula un score agregado de la corrida completa
    ...
    py_files = [f for f in files if Path(f.get("filename", "unknown")).suffix.lower() == ".py"]
    ...
    try:
        if py_files and ("flake8" in req.tools or "pylint" in req.tools):
            for f in py_files:
                tmp_by_filename[f.get("filename", "unknown")] = save_temp(f.get("content", ""), ".py")
            ...
            if "pylint" in req.tools:
                ...
                for fname, tmp in tmp_by_filename.items():
                    ...
                    results[fname]["tools_used"].append("pylint")
    finally:
        for tmp in tmp_by_filename.values():
            safe_remove(tmp)
            ...
            issues.append(
                {
                    ...
                    "severity": "warning",
                    "code": "W001",
                    "message": "assert puede desactivarse con -O",
                }
            )
            ...
            issues.append(
                {
                    ...
                    "severity": "warning",
                    "code": "W002",
                    "message": "except genérico captura todo",
                }
            )
...
def _run_flake8_batch(filepaths: list[str]) -> dict[str, list[dict]]:
    """Igual que _run_flake8 pero UN solo subprocess para todos los filepaths —
    ver docstring de /analyze/project para el porqué."""
    by_path: dict[str, list[dict]] = {fp: [] for fp in filepaths}
    if not filepaths:
        return by_path
    try:
        result = subprocess.run(
...
def _run_pylint_batch(filepaths: list[str]) -> dict[str, list[dict]]:
    """Igual que _run_pylint pero UN solo subprocess para todos los filepaths.
    No devuelve score: pylint calcula un solo score agregado para la corrida
    completa cuando se le pasan varios archivos, no uno por archivo — ver
    docstring de /analyze/project."""
    by_path: dict[str, list[dict]] = {fp: [] for fp in filepaths}
    if not filepaths:
        return by_path
    try:
        result = subprocess.run(
            [sys.executable, "-m", "pylint", "--output-format=json", "--disable=C0114,C0115", *filepaths],
            capture_output=True,
            text=True,
            timeout=60,
        )
