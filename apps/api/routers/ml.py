...
async def analyze_ml(req: AnalyzeMLRequest):
    ...
    try:
        ...
        add_log(
            "info", f"ML analizado: {filename} — " f"{len(result['libraries'])} libs, {len(result['pipeline'])} etapas"
        )
...
def _detect_ml_issues(content: str, libraries: list[dict]) -> list[dict]:
    ...
    _code_lines = [ln for ln in content.splitlines() if not ln.strip().startswith(("import ", "from "))]
    _code_only = "\n".join(_code_lines)
        ...
        issues.append(
            {
                "severity": "warning",
                "category": "reproducibilidad",
                "message": "KFold sin random_state",
...
def _ml_score(content: str, result: dict) -> tuple[int, list[str]]:
    ...
    if any(s["id"] == "evaluacion" for s in pipeline):
        score += 5
    if any(s["id"] == "split" for s in pipeline):
        score += 5
    if re.search(r"cross_val_score|KFold", content):
        score += 10
    if re.search(r"random_state\s*=", content):
        score += 5
    if len(pipeline) >= 4:
        score += 5
