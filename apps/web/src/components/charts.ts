      ...
      datasets: [
        {
          label: 'ms',
          data: [],
          backgroundColor: 'rgba(61,158,255,.4)',
          borderColor: '#3d9eff',
          borderWidth: 1,
          borderRadius: 3,
        },
  ...
  charts['dist'] = new Chart(distCtx, {
    ...
    data: {
      ...
      datasets: [
        {
          data: [0, 0, 0, 0],
          backgroundColor: ['#ff3366', '#ffb627', '#3d9eff', '#00f5a0'],
          borderWidth: 0,
          hoverOffset: 4,
        },
      ],
    ...
    data: {
      ...
      datasets: [
        ...
        },
        {
          ...
          data: [],
          borderColor: '#00f5a0',
          backgroundColor: 'rgba(0,245,160,.08)',
          tension: 0.4,
          fill: true,
          pointRadius: 2,
          borderWidth: 1.5,
...
export function renderRTChart(): void {
  ...
  ;(c.data.datasets[0] as any).backgroundColor = apis.map((a) =>
    a.ms! < 200 ? 'rgba(0,245,160,.4)' : a.ms! < 1000 ? 'rgba(255,182,39,.4)' : 'rgba(255,51,102,.4)',
  )
  // eslint-disable-next-line @typescript-eslint/no-explicit-any
  ;(c.data.datasets[0] as any).borderColor = apis.map((a) =>
    a.ms! < 200 ? '#00f5a0' : a.ms! < 1000 ? '#ffb627' : '#ff3366',
  )
  c.update()
...
export function renderComplexityBars(): void {
  ...
  if (!files.length) {
    el.innerHTML = '<div style="color:var(--muted);font-size:.72rem">Sin datos</div>'
    return
  }
