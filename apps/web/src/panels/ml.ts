// ══════════════════════════════════════════
//  Sythrall — ML/DL Panel
// ══════════════════════════════════════════
// panels/ml.ts
import type { CodeFile, MLAnalysisResult, NotebookEnv, NotebookEnvId } from '../types'
...
const NOTEBOOK_ENVS: NotebookEnv[] = [
  ...
  },
  {
    ...
    description:
      'Framework para notebooks interactivos en el navegador con soporte nativo de celdas de código y carga de librerías',
...
export function clientMLAnalysis(f: CodeFile): MLAnalysisResult {
  ...
  const catMap: Record<string, string> = {
    ...
    icecream: 'debug',
    cython: 'rendimiento',
    pyximport: 'rendimiento',
  }
  const libraries = Object.entries(libNames)
    .filter(([k]) => new RegExp(`\\b${k}\\b`).test(content))
    .map(([k, name]) => ({
      name,
      category: catMap[k] ?? 'otros',
      color: catMap[k] === 'rendimiento' ? '#ffd43b' : '#4a5880',
      import: k,
      alias: null,
      version: null,
    }))
