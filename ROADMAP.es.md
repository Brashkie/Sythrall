...
- [x] **Code Quality, tres porciones portadas** — `maintainability.rs::halstead_metrics()`: los 5 componentes de Halstead (Fase 22), Rust-only a propósito (sin fallback Python — mismo límite que MI/CC/raw tienen desde que la Fase 11 reemplazó a `radon`, no una excepción a la migración, la forma correcta para una feature que nunca tuvo doble implementación); `smells.rs`/`static_parser.py`: 5 chequeos de smells estructurales, construidos en ambos lenguajes el mismo día, el caso general que apunta la corrección de Rust-first; scores de Project Health (Fase 22) que agregan `security_findings`/`structural_smells` a nivel de proyecto, agregación aritmética sobre resultados que el motor Rust-first ya calculaba, no lógica de análisis nueva. Smells de nombres/arquitectura y el historial de tendencias por proyecto quedan en cola
