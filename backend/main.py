"""
CodeWatch PRO — Backend FastAPI v4.1
...
from shared import add_log, now, LOG_HISTORY, API_HISTORY, LIB_FLAGS, LIB_VERSIONS

# ── Imports condicionales ────────────────────────────────────────────────────
try:    import flake8;               LIB_FLAGS["HAS_FLAKE8"]   = True
except ImportError: pass
try:    import pylint;               LIB_FLAGS["HAS_PYLINT"]   = True
except ImportError: pass
try:
...
try:    import numpy as np;          LIB_FLAGS["HAS_NUMPY"]    = True
except ImportError: pass
try:    import pandas as pd;         LIB_FLAGS["HAS_PANDAS"]   = True
except ImportError: pass
try:    import sklearn;              LIB_FLAGS["HAS_SKLEARN"]  = True
except ImportError: pass
try:    import torch;                LIB_FLAGS["HAS_TORCH"]    = True
except ImportError: pass
try:    import tensorflow as tf;     LIB_FLAGS["HAS_TF"]       = True
except ImportError: pass
try:    import scipy;                LIB_FLAGS["HAS_SCIPY"]    = True
except ImportError: pass
try:    import cv2;                  LIB_FLAGS["HAS_CV2"]      = True
except ImportError: pass
try:    import plotly;               LIB_FLAGS["HAS_PLOTLY"]   = True
except ImportError: pass
try:    import polars as pl;         LIB_FLAGS["HAS_POLARS"]   = True
except ImportError: pass
try:    import lightgbm as lgb;      LIB_FLAGS["HAS_LGB"]      = True
except ImportError: pass
try:    import spacy;                LIB_FLAGS["HAS_SPACY"]    = True
except ImportError: pass
try:    import icecream;             LIB_FLAGS["HAS_ICECREAM"] = True
except ImportError: pass
try:    import Cython;               LIB_FLAGS["HAS_CYTHON"]   = True
except ImportError: pass
...
async def lifespan(app: FastAPI):
    ...
    add_log("info", f"   polars={'✓' if HAS_POLARS else '✗'}  opencv={'✓' if HAS_CV2 else '✗'}  lightgbm={'✓' if HAS_LGB else '✗'}")
    add_log("info", f"   scipy={'✓' if HAS_SCIPY else '✗'}  spacy={'✓' if HAS_SPACY else '✗'}  cython={'✓' if HAS_CYTHON else '✗'}")
    yield
...
async def health():
    return {"status": "ok", "ts": now(), "capabilities": {
        "flake8":     HAS_FLAKE8,
        "pylint":     HAS_PYLINT,
        "radon":      HAS_RADON,
        "numpy":      HAS_NUMPY,
