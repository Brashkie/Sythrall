...
def _import_graph_to_mermaid(
    ...
    for n in nodes:
        ...
        if n["id"] not in targets and n["id"] in sources:
            mermaid += f"    style {nid} fill:#3d9eff20,stroke:#3d9eff\n"
        elif n["id"] not in sources:
            mermaid += f"    style {nid} fill:#00f5a020,stroke:#00f5a0\n"
...
def _call_graph_to_mermaid(nodes: list[dict], edges: list[dict]) -> str:
    ...
    for n in nodes:
        if n.get("level") == "expensive":
            nid = _safe_id(n["id"])
            mermaid += f"    style {nid} fill:#ff336620,stroke:#ff3366\n"
        elif n.get("level") == "moderate":
            nid = _safe_id(n["id"])
            mermaid += f"    style {nid} fill:#ffb62720,stroke:#ffb627\n"
...
def _heatmap_to_mermaid(functions: list[dict]) -> str:
    ...
    for fn in functions:
        ...
        fill = fn["cc_color"] + "20"
        stroke = fn["cc_color"]
        mermaid += f"    style {fnid} fill:{fill},stroke:{stroke}\n"
...
async def generate_project_graph(req: ProjectGraphRequest) -> dict[str, Any]:
    ...
    try:
        ...
        # Recolectar archivos de código con paths completos
        CODE_EXTS = {".py", ".ts", ".js", ".tsx", ".jsx", ".c", ".cpp", ".h", ".hpp"}
        files_for_graph: list[dict] = []
        ...
        for root, dirs, files in os.walk(project_dir):
            ...
            for fname in files:
                ...
                if ext not in CODE_EXTS:
                    continue
                # Path relativo al proyecto (e.g. "frontend/app.ts")
                rel_path = os.path.relpath(full_path, project_dir).replace("\\", "/")
                try:
                    with open(full_path, encoding="utf-8", errors="replace") as f:
                        content = f.read()
                    if content.strip():
                        files_for_graph.append({"filename": rel_path, "content": content})
                except Exception:
                    pass
