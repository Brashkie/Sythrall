...
- [x] **Code Quality, three slices ported** — `maintainability.rs::halstead_metrics()`: the 5 Halstead components (Phase 22), Rust-only by design (no Python fallback — same boundary MI/CC/raw have had since Phase 11 replaced `radon`, not an exception to the migration, the correct shape for a feature that was never dual-implemented to begin with); `smells.rs`/`static_parser.py`: 5 structural smell checks, built in both languages the same day, the general case the Rust-first correction targets; Project Health scores (Phase 22) aggregate `security_findings`/`structural_smells` to project level, orchestration/arithmetic over results the Rust-first engine already computed, not new analysis logic. Naming/architecture smells and per-project trend history remain queued
