...
async function post<T>(path: string, body: unknown): Promise<T> {
  ...
  try {
    ...
    if (!res.ok) {
      ...
      try { const b = await res.json(); msg = b.detail ?? b.message ?? msg } catch { /* noop */ }
      throw new Error(msg)
    }
  ...
  return new Promise((resolve, reject) => {
    ...
    xhr.addEventListener('load', () => {
      if (xhr.status >= 200 && xhr.status < 300) {
        ...
        try { const b = JSON.parse(xhr.responseText); reject(new Error(b.detail ?? `Error ${xhr.status}`)) }
        catch { reject(new Error(`Error del servidor: ${xhr.status}`)) }
      }
    })
...
export interface ProjectTreeNode {
  ...
  size?: number; size_fmt?: string; extension?: string
  is_code?: boolean; modified?: string
  children?: ProjectTreeNode[]
  truncated?: boolean; error?: string
}

export interface ProjectInfo {
  total_files: number; total_size: number; total_size_fmt: string
  code_files: number; by_extension: Record<string, number>; created_at: string
}

export interface UploadResult {
  project_id: string; project_name: string
  type: 'files' | 'folder' | 'zip'
  total_files: number; tree: ProjectTreeNode
  info?: ProjectInfo
  errors?: Array<{ file: string; reason: string }>
  extracted?: number; skipped?: number
}

export interface ProjectSummary {
  project_id: string; total_files: number
  total_size: number; total_size_fmt: string
  code_files: number; by_extension: Record<string, number>; created_at: string
}

export interface FileContent {
  path: string; content: string; size: number; extension: string
}
...
export const api = {
  ...
  checkUrls: (urls: string[], timeout = 10) =>
    post<{ results: ApiResult[] }>('/analyze/api', { urls, timeout }),
  ...
  graphTypes: () =>
    get<{ types: GraphType[] }>('/analyze/graph/types'),
  ...
  listProjects: () =>
    get<{ projects: ProjectSummary[]; total: number }>('/api/upload/projects'),

  getProjectTree: (projectId: string) =>
    get<{ project_id: string; tree: ProjectTreeNode; info: ProjectInfo }>(
      `/api/upload/projects/${projectId}/tree`
    ),
  ...
  deleteProject: (projectId: string) =>
    fetch(getApiBase() + `/api/upload/projects/${projectId}`, { method: 'DELETE' })
  ...
  browserPing: async (url: string): Promise<ApiResult> => {
    ...
    try {
      ...
      r.ms = Date.now() - t0; r.status = 'ok'; r.code = '2xx'
    } catch (e: unknown) {
      r.ms = Date.now() - t0
  ...
  staticParse: (filename: string, content: string) =>
    post<StaticParseResult>('/static/parse', { filename, content }),

  staticParseProject: (files: Array<{ filename: string; content: string }>) =>
    post<StaticProjectResult>('/static/parse-project', { files }),

  staticBigO: (filename: string, content: string) =>
    post<BigOResult>('/static/bigO', { filename, content }),
  ...
  intelLint: (filename: string, content: string) =>
    post<IntelLintResult>('/intel/lint', { filename, content }),
  ...
  intelCompletions: (filename: string, content: string, prefix = '') =>
    post<{
      symbols: Array<{
        ...
        documentation: string | null; insert_text: string
        sort_text: string; line: number
      }>
      total: number; filename: string; prefix: string
    }>('/intel/completions', { filename, content, prefix }),

  intelRename: (filename: string, content: string, symbol_name: string, new_name: string) =>
    post<{
      valid:    boolean
      edits:    Array<{ range: { startLineNumber: number; startColumn: number; endLineNumber: number; endColumn: number }; newText: string; kind: string; preview: string }>
      total:    number
      old_name: string
      new_name: string
      filename: string
