      ...
      ${st.projects
        ...
        <div class="up-recent-item">
          ...
          <div class="up-recent-info">
            <span class="up-recent-id">${esc(p.project_id.slice(0, 8))}…</span>
...
function attachUploadEvents(el: HTMLElement): void {
  ...
  el.addEventListener('click', async (e) => {
    ...
    if (fileRow && st.activeResult) {
      ...
      await loadFileInEditor(st.activeResult.project_id, selectedPath)
      return
    }
    ...
    if (useBtn) {
      e.stopPropagation()
      toast('📂 Proyecto disponible — selecciona archivos del árbol', 'ok')
      return
    }
  })
...
// ─── Cargar archivo en Monaco ─────────────────────────────────────────────────

async function loadFileInEditor(projectId: string, filePath: string): Promise<void> {
  try {
    const fileContent = await api.getFileContent(projectId, filePath)

    // Importar dinámico para no crear dependencia circular
    const { loadFileInEditor: monacoLoad } = await import('../components/editor')
    const ext = fileContent.extension ?? filePath.split('.').pop() ?? ''

    monacoLoad({
      id: `upload-${projectId}-${filePath}`,
    ...
    const { switchTab } = await import('../components/app')
    switchTab('editor')
