// ══════════════════════════════════════════
//  CodeWatch PRO — Editor Intelligence
 ...
 */
export function initEditorIntelligence(
  ...
  _disposables.push(
    editor.onDidChangeModelContent(() => _scheduleAnalysis())
  )
  _disposables.push(
...
function _applyMarkers(): void {
  ...
  monaco.editor.setModelMarkers(_model, 'codewatch-intel', dedup)
}

function _clearAllMarkers(): void {
  _markers.fast = []; _markers.heavy = []
  if (_model) monaco.editor.setModelMarkers(_model, 'codewatch-intel', [])
}
...
function _toMonacoMarker(raw: RawMarker): monaco.editor.IMarkerData {
  ...
  return {
    startLineNumber: raw.startLineNumber, startColumn: raw.startColumn,
    endLineNumber:   raw.endLineNumber,   endColumn:   raw.endColumn,
    message: raw.message, severity: sev, source: raw.source, code: raw.code,
  }
}
...
async function _fetchHover(
  ...
  try {
    ...
    const res  = await fetch(getApiBase() + '/intel/hover', {
      method: 'POST', headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ filename, content, line: position.lineNumber, column: position.column, symbol_name: word.word }),
      signal: ctrl.signal,
    })
    clearTimeout(tid)
    ...
    if (data.range) {
      hover.range = new monaco.Range(
        data.range.startLineNumber, data.range.startColumn ?? 1,
        data.range.endLineNumber,   data.range.endColumn   ?? 999,
      )
    }
    return hover
  } catch { return null }
...
export async function fetchCallGraph(filename: string, content: string): Promise<CallGraphData | null> {
  try {
    ...
    const nodes = data.functions.map(fn => ({
      id: fn.name, label: fn.name,
      kind: fn.is_async ? 'async_function' : 'function',
      line: fn.line, big_o: fn.big_o,
    }))
...
interface RawMarker {
  ...
  message: string; severity: number; source: string; code: string
}
...
interface DefinitionResponse {
  found: boolean; symbol: string; filename: string
  definitions: Array<{ line: number; column: number; end_line?: number; kind: string; signature: string; docstring: string }>
}

interface ReferencesResponse {
  symbol: string; filename: string
  references: Array<{ line: number; column: number; kind: string; preview: string }>
  definition_line: number | null; total: number
}

interface CompletionSymbol {
  label: string; kind: string; detail: string
  documentation: string | null; insert_text: string; sort_text: string; line: number
}

interface CompletionsResponse {
  symbols: CompletionSymbol[]; total: number; filename: string; prefix: string
}

interface RenameEdit {
  range: { startLineNumber: number; startColumn: number; endLineNumber: number; endColumn: number }
  newText: string; kind: string; preview: string
}

interface RenameResponse {
  valid: boolean; edits: RenameEdit[]; total: number
  old_name: string; new_name: string; filename: string; error?: string
}
